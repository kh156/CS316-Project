...
@Entity
public class Problem extends Model implements Comparable<Problem>{
    ...
    @ManyToOne
    public Textbook forum;
    
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "topic")
    public List<Post> posts;
    
    
    public Problem(Textbook forum, User by, String sectionIdx, String problemIdx, String content) {
        this.forum = forum;        
    ...
    public List<Post> getPosts(int page, int pageSize) {
        List<Post> list = Post.find("topic = ? order by likesNum desc, dislikesNum asc, postedAt asc", this).fetch(page, pageSize);
        return list;
    }
    ...
    public Long getVoicesCount() {
        return User.count("select count(distinct u) from User u, Problem t, Post p where p.postedBy = u and p.topic = t and t = ?1", this);
    }
